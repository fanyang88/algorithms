...
var findContentChildren = function(g, s) {
    ...
    s= s.sort((a,b) => a-b);
    var i=0, count=0;
    // for each kid we loop till find the cookie it desired, then move to next kid
    for(var content of g) {
        while(i<s.length && s[i] < content) i++; 
        //now child is content with s[i];
        if(i===s.length)  break;
        count++;
