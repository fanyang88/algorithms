...
AutocompleteSystem.prototype.insert = function(str, times) {
    ...
    for(var chr of str) {
        ...
        // also insert related string
        cur.related[str] = cur.related[str] ? cur.related[str]+times : times;
    }
...
AutocompleteSystem.prototype.input = function(c) {
    if(c === '#') {  // come to an end
        this.insert(this.line, 1);
        this.line = '';
        return [];
    }
    var cur = this.root;
    this.line += c;
    for(var chr of this.line) {
        if(!cur.children[chr]) return []; 
        cur = cur.children[chr];
    }
    // sort related words
    var strs = Object.keys(cur.related);
    strs.sort((a,b) => {
        if(cur.related[a] === cur.related[b]) {
            if(a>b)  return 1;
            if(a<b)  return -1;
            return 0;
        }
        return cur.related[b] - cur.related[a];
    });
