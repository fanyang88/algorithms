...
var candyCrush = function(board) {
    var m = board.length, n = board[0].length, continueCrush = false;
    // Mark candies can be crushed horizonally to be neg
    for(var i=0; i<m; i++) {
        for(var j=0; j<n-2; j++) {
            var val = Math.abs(board[i][j]);
    ...
    // Mark candies can be crushed vertically to be neg
    for(var j=0; j<n; j++) {
        for(var i=0; i<m-2; i++) {
            var val = Math.abs(board[i][j]);
    ...
    for(var j=0; j<n; j++) {
        ...
        // change r to 0 row to be 0 in this col
        for(var i=r; i>=0; i--) {
            board[i][j] = 0;
        }
    }
    return continueCrush ? candyCrush(board) : board;
};
