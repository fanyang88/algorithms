...
var largestIsland = function(grid) {
    var map= {}, color = 2, area=[0], m = grid.length, n = grid[0].length; 
    var dir= [[0, 1], [0, -1], [1, 0], [-1, 0]];
    // step1: get how many islands and each area and strore in a map
    for(var i=0; i<m; i++) {
        for(var j=0; j<n; j++) {
            if(grid[i][j] === 1) {
                // color grid[i][j] and its neighbor in same island to be same color 
        ...
        for(var j=0; j<n; j++) {
            if(grid[i][j] === 0) {
                ...
                for(var k=0; k<4; k++) {
                    ...
                    if(x<m && x>=0 && y>=0 && y<grid[0].length && grid[x][y] !== 0 && !set.has(grid[x][y])) {
