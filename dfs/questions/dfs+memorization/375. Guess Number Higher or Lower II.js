...
dfs = function(s, e, map) {
    if(s >= e) return 0;    // has only 1 element, no need to guess, so no cost
    
    if(map[s+':'+e])  return map[s+':'+e];
    var answer = Infinity;
    for(var k=s; k<=e; k++) {
        var val = k + Math.max(dfs(s, k-1, map), dfs(k+1, e, map)); // max cost = cost from left, or cost from right
