 ...
 * @param {number[]} nums
 * @return {boolean}
 init the diagnal first
 fomula is: T[i][j].first = max(T[i+1][j].second + val[i], T[i][j-1].second + val[j]) // pick left or right 
 e.g: [1, 5, 2]
 left: score for p1, score for p2
 (1,0)  (max(0+1, 0+5), since we picked right, second=(2,0).first)
        (5,0)
              (2,0)
 */
var PredictTheWinner = function(nums) {
    var n = nums.length, dp = [];
    for(var i=0; i<n; i++) {
        dp[i] = [];
        dp[i][i] = [nums[i], 0];
    } 
